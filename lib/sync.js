 ...
 *
 * JEDA_ESCROW_MS bisa disetel lewat env tanpa deploy ulang. get_escrow_detail
 * punya batas laju yang jauh lebih ketat daripada endpoint pesanan — jeda 350
 * ms (bawaan beruntun) ditolak setelah beberapa panggilan. Kalau penarikan
 * masih sering kena tolak, naikkan nilainya; kalau lancar terus, turunkan.
 */
export async function tarikEscrow(shopId, { batas = null, jeda = null } = {}) {
  const jedaMs = jeda ?? (Number(process.env.JEDA_ESCROW_MS) || 800);
  /**
   * BATAS_ESCROW menentukan berapa pesanan per toko per putaran.
   *
   * Angkanya harus disesuaikan dengan jeda, bukan dibiarkan tetap.
   * Penjadwal berjalan tiap 5 menit dan tidak saling tumpang tindih.
   * Kalau batas x (jeda + waktu balasan) jauh LEBIH PENDEK dari 5 menit,
   * putarannya selesai lalu menganggur sampai tik berikutnya — dan
   * mempercepat jeda tidak ada gunanya sama sekali.
   *
   * Pada jeda 300 ms, sekali panggil makan sekitar 0,6 detik, jadi batas
   * 500 mengisi hampir seluruh lima menit itu. Pada jeda 800 ms bawaan,
   * 150 sudah pas.
   */
  const batasN = batas ?? (Number(process.env.BATAS_ESCROW) || 150);
  const token = await tokenHidup(shopId);
  /**
   * Yang ditarik:
    ...
    `SELECT o.id, o.order_sn FROM orders o
     ...
     WHERE o.shop_id = $1
       AND (e.order_id IS NULL
            ...
            -- Baris lama yang ditarik sebelum kolom ads_topup ada. Nilainya
            -- basi: Shopee sempat memotong lebih banyak lalu membetulkannya
            -- belakangan, dan kita tidak pernah kembali mengambil versi yang
            -- sudah benar. NULL di sini = belum pernah disegarkan.
            OR e.ads_topup IS NULL)
       /**
        * JANGAN TAMBAHKAN "OR e.release_at IS NULL" DI SINI.
        * (Tanpa backtick — komentar ini ada DI DALAM template literal SQL,
        *  jadi satu backtick saja akan menutup string-nya dan build gagal.)
        *
        * Sempat ada, dan akibatnya penyegaran MACET TOTAL tanpa satu pun
        * galat: get_escrow_detail TIDAK memuat tanggal pencairan (sudah
        * dibuktikan — kandidat_stempel_waktu balik kosong), jadi baris
        * ber-release_at kosong ditarik, disimpan, tanggalnya tetap kosong,
        * lalu terjaring lagi di putaran berikutnya. Selamanya. Jatah 150
        * habis untuk pekerjaan yang tidak mungkin selesai, dan layar
        * melaporkan "149 escrow tersimpan" seolah semuanya sehat.
        *
        * Tanggal pencairan diisi penyapuan terpisah di bawah, bukan di sini.
        */
       AND o.status IN ('COMPLETED','TO_CONFIRM_RECEIVE')
     /**
      * PESANAN YANG ANGKANYA JANGGAL DIDAHULUKAN.
      *
      * Menyegarkan 50 ribu baris makan waktu berhari-hari. Yang mengubah
      * angka laporan cuma sebagian kecil — pesanan yang potongannya di atas
      * 30%, karena di situlah isi saldo iklan tersembunyi. Dengan
      * mendahulukannya, laporan jadi benar dalam hitungan jam, bukan hari;
      * sisanya tetap dikerjakan, hanya belakangan.
      *
      * Dipakai buyer_paid (bukan omzet dari order_items) supaya tidak perlu
      * JOIN mahal — dua kolomnya sudah ada di order_escrow.
      */
     ORDER BY CASE WHEN COALESCE(e.buyer_paid,0) > 0
                    AND (e.buyer_paid - e.escrow_amount) / e.buyer_paid > 0.30
                   THEN 0 ELSE 1 END,
              o.created_time DESC
     LIMIT $2`, [shopId, batasN]);

  let n = 0, gagal = 0, berhenti = null;
  await beruntun(perlu, async (o) => {
    if (berhenti) return;                    // batch sudah disetop, lewati sisanya
    try {
      ...
      await q(
        ...
        [o.id, Number(inc.original_price) || 0, Number(inc.buyer_total_amount) || 0,
         ...
         /**
          * Isi Saldo Otomatis iklan. Shopee sudah memotongnya dari
          * escrow_amount, padahal ini BUKAN biaya — uangnya pindah ke
          * dompet iklan lalu keluar lagi dan tercatat di ad_spend.
          *
          * `|| 0` disengaja, bukan `?? 0`: kalau field-nya tidak ada sama
          * sekali di balasan, nilainya harus 0 dan BUKAN null. Null membuat
          * baris ini terjaring lagi di penarikan berikutnya, berputar
          * selamanya tanpa pernah selesai.
          */
         Number(inc.ads_escrow_top_up_fee_or_technical_support_fee) || 0]);
      n++;
    } catch (e) {
      /**
       * Batas laju TIDAK lagi membatalkan seluruh batch.
       *
       * Dulu galat ini dilempar ke atas, lalu ditelan `.catch(() => 0)` di
       * sinkronSemua. Akibatnya satu penolakan membuang 147 pesanan sisanya
       * tanpa jejak apa pun di layar — penarikan maju tiga baris per putaran
       * dan praktis tidak akan pernah selesai.
       *
       * Sekarang: mundur sejenak, coba sekali lagi. Kalau masih ditolak,
       * barulah batch ini disudahi dengan baik-baik — sisanya dikerjakan
       * putaran berikutnya, dan alasannya tercatat.
       */
      if (e.kind === 'rate_limit') {
        await tidur(Math.max(jedaMs * 8, 8000));
        try {
          const json = await call(EP.escrowDetail, {
            accessToken: token, shopId, params: { order_sn: o.order_sn },
          });
          const r2 = json.response || {};
          const inc2 = r2.order_income || {};
          const d2 = r2.escrow_release_time ?? inc2.escrow_release_time
                  ?? r2.payout_time ?? r2.release_time ?? inc2.release_time ?? null;
          const cair2 = d2 && Number(d2) > 1e9 ? new Date(Number(d2) * 1000) : null;
          await q(
            `INSERT INTO order_escrow (order_id, gross_amount, buyer_paid, commission_fee,
                                       service_fee, seller_discount, shopee_subsidy, escrow_amount,
                                       release_at, ads_topup)
             ...
             ON CONFLICT (order_id) DO UPDATE SET
               ...
               release_at     = COALESCE(EXCLUDED.release_at, order_escrow.release_at),
               fetched_at     = now()`,
            [o.id, Number(inc2.original_price) || 0, Number(inc2.buyer_total_amount) || 0,
             Number(inc2.commission_fee) || 0, Number(inc2.service_fee) || 0,
             Number(inc2.seller_discount) || 0, Number(inc2.shopee_discount) || 0,
             Number(inc2.escrow_amount) || 0, cair2,
             Number(inc2.ads_escrow_top_up_fee_or_technical_support_fee) || 0]);
          n++;
        } catch (e2) {
          berhenti = e2.kind === 'rate_limit'
            ? 'batas laju Shopee — naikkan JEDA_ESCROW_MS'
            : e2.message;
        }
        return;
      }
      // Galat lain: pesanan ini dilewati, batch jalan terus. Dulu galat
      // seperti ini hilang tanpa bekas; sekarang minimal terhitung.
      gagal++;
    }
  }, jedaMs);

  /**
   * Escrow tidak pernah muncul di ringkasan sinkron, jadi selama ini
   * kemacetannya tak terlihat di layar mana pun. Sekarang dicatat —
   * tapi hanya kalau ada yang perlu diketahui, supaya log tidak banjir.
   */
  if (gagal || berhenti) {
    await log('escrow',
      `Toko ${shopId}: ${n} escrow tersimpan`
      + (gagal ? `, ${gagal} pesanan dilewati` : '')
      + (berhenti ? `, batch disudahi: ${berhenti}` : ''),
      { shopId, ok: !berhenti }).catch(() => {});
  }
  return n;
}
...
export async function sinkronSemua({ hariKeBelakang = 3, bidangWaktu = 'update_time' } = {}) {
  ...
  for (const t of toko) {
    try {
      ...
      hasil.push({ shopId: String(t.shop_id), pesanan: p, escrow: 0, ok: true });
    } catch (err) {
  ...
  /**
   * ESCROW DIKERJAKAN PARALEL ANTAR TOKO.
   *
   * Dulu escrow ikut di dalam perulangan toko di atas, jadi seluruh
   * sistem hanya pernah punya SATU panggilan berjalan pada satu waktu —
   * toko demi toko, pesanan demi pesanan. Dengan jeda 0,8 detik plus
   * waktu balasan Shopee, lajunya mentok di sekitar 37 pesanan/menit.
   * Untuk menyegarkan 50 ribu baris, itu berhari-hari.
   *
   * Tiap toko memakai access token sendiri dan batas laju Shopee
   * dihitung per toko, jadi menjalankannya bersamaan tidak menambah
   * tekanan pada satu toko pun. Yang bertambah hanya beban jaringan
   * kita sendiri.
   *
   * Dibatasi empat sekaligus, bukan semua: kalau kedelapan toko
   * ditembakkan berbarengan, Postgres kebagian delapan penulis
   * bersamaan sementara aplikasi masih melayani halaman. Empat sudah
   * memberi hampir seluruh percepatannya tanpa mengganggu yang lain.
   */
  const PARALEL = Number(process.env.PARALEL_ESCROW) || 4;
  const antre = [...toko];
  const petaHasil = new Map(hasil.map((h) => [h.shopId, h]));

  async function pekerja() {
    for (;;) {
      const t = antre.shift();
      if (!t) return;
      const n = await tarikEscrow(t.shop_id).catch(async (err) => {
        // Dulu `.catch(() => 0)` polos — galat escrow hilang tanpa bekas
        // dan penarikan macet berbulan-bulan tanpa ada yang tahu.
        await log('escrow', `Toko ${t.shop_id} gagal: ${err.message}`,
                  { shopId: t.shop_id, ok: false }).catch(() => {});
        return 0;
      });
      const h = petaHasil.get(String(t.shop_id));
      if (h) h.escrow = n;
    }
  }
  await Promise.all(Array.from({ length: Math.min(PARALEL, antre.length) }, pekerja));
