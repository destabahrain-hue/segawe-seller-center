...
export async function tarifPlatform({ hari = 180 } = {}) {
  const rows = await q(`
    ...
    SELECT p.shop_id,
           ...
           SUM(COALESCE(e.escrow_bersih,0))   AS diterima,
...
export async function ringkasan({ dari, sampai, shopId = null }) {
  const rows = await q(`${CTE}
    SELECT
      ...
      SUM(CASE WHEN COALESCE(e.escrow_amount,0) > 0 THEN e.escrow_bersih ELSE 0 END)   AS diterima_bersih,
  ...
  const toko = rows.map((r) => {
    ...
    const diterima   = Number(r.diterima) || 0;
    /**
     * Dua angka, dua arti, jangan tertukar:
     *   diterima       = uang tunai yang benar-benar masuk  → kartu "Dana diterima"
     *   diterimaBersih = sebelum dipotong isi saldo iklan   → dasar hitung BIAYA
     * Kalau biaya dihitung dari `diterima`, isi saldo iklan terhitung sebagai
     * biaya platform padahal nanti terhitung lagi sebagai iklan.
     */
    const diterimaBersih = Number(r.diterima_bersih) || 0;
    ...
    const biayaNyata     = Math.max(omzetCair - diterimaBersih, 0);
 ...
 * ── Analisa SKU bocor ──
 *
 * "Bocor" = SKU yang terjual tapi hasilnya justru menggerus uang. Ada tiga
 * bentuk yang berbeda, dan memisahkannya penting karena tindakannya beda:
 *
 *   rugi       laba per SKU negatif — harga jual tidak menutup HPP + biaya
 *   tipis      laba positif tapi margin di bawah ambang; sekali diskon
 *              atau ongkir naik, langsung rugi
 *   buta       HPP-nya nol/belum tertaut — labanya TERLIHAT besar padahal
 *              belum dihitung. Ini bocor yang paling berbahaya karena
 *              menyamar jadi kabar baik
 *
 * BIAYA IKLAN TIDAK DIBAGI KE SKU. ad_spend tersimpan per toko per hari,
 * bukan per produk, jadi membaginya secara proporsional akan membebani SKU
 * yang tidak pernah diiklankan dengan biaya iklan SKU lain — dan angka itu
 * akan terlihat resmi padahal karangan. Yang dilaporkan di sini adalah laba
 * SEBELUM iklan; beban iklan tokonya ditampilkan terpisah sebagai konteks.
 */
export async function skuBocor({ dari, sampai, shopId = null, ambangTipis = 0.10 }) {
  const rows = await q(`${CTE},
    -- Biaya platform per BARIS pesanan, dibagi menurut porsi omzet baris
    -- itu di dalam pesanannya. Ini metodologi yang sama dengan laporan
    -- bulanan: biaya melekat ke pesanan, bukan ke barang, jadi satu-satunya
    -- pembagian yang jujur adalah proporsional.
    biaya_pesanan AS (
      SELECT p.id, p.shop_id, p.omzet,
             CASE WHEN COALESCE(e.escrow_amount,0) > 0
                  THEN GREATEST(p.omzet - e.escrow_bersih, 0)
                  ELSE NULL END AS biaya_nyata
        FROM per_pesanan p
        LEFT JOIN order_escrow e ON e.order_id = p.id
    ),
    -- hpp_baris berisi SATU BARIS PER KOMPONEN (paket bundling diuraikan),
    -- jadi harus dijumlah dulu per baris pesanan. Tanpa ini, SKU paket
    -- berisi tiga komponen akan menggandakan omzetnya tiga kali saat
    -- digabungkan — angka yang salah tapi terlihat masuk akal.
    hpp_oi AS (
      SELECT oi_id, SUM(hpp) AS hpp FROM hpp_baris GROUP BY oi_id
    ),
    baris AS (
      SELECT t.oi_id, t.id, t.shop_id,
             COALESCE(ms.code, NULLIF(TRIM(t.model_sku),''),
                      NULLIF(TRIM(t.item_sku),''), '(tanpa SKU)') AS code,
             COALESCE(ms.name, t.item_name, '(tanpa nama)')       AS name,
             t.image_url,
             ms.id IS NOT NULL         AS tertaut,
             t.qty                     AS unit,
             t.qty * t.price           AS omzet,
             COALESCE(h.hpp, 0)        AS hpp,
             bp.omzet                  AS omzet_pesanan,
             bp.biaya_nyata
        FROM tertaut t
        LEFT JOIN master_sku ms ON ms.id = t.msku
        LEFT JOIN hpp_oi h ON h.oi_id = t.oi_id
        LEFT JOIN biaya_pesanan bp ON bp.id = t.id
    )
    SELECT b.shop_id, s.shop_name, b.code,
           MIN(b.name)       AS name,
           MIN(b.image_url)  AS gambar,
           BOOL_OR(b.tertaut) AS tertaut,
           SUM(b.unit)       AS unit,
           COUNT(*)::int     AS baris,
           SUM(b.omzet)      AS omzet,
           SUM(b.hpp)        AS hpp,
           -- Porsi biaya baris ini. Kalau pesanannya belum cair, biayanya
           -- belum diketahui dan dijumlah terpisah supaya bisa ditaksir
           -- dengan tarif toko di JavaScript.
           SUM(CASE WHEN b.biaya_nyata IS NOT NULL AND b.omzet_pesanan > 0
                    THEN b.biaya_nyata * (b.omzet / b.omzet_pesanan)
                    ELSE 0 END)                                   AS biaya_nyata,
           SUM(CASE WHEN b.biaya_nyata IS NULL THEN b.omzet ELSE 0 END) AS omzet_belum
      FROM baris b
      LEFT JOIN shops s ON s.shop_id = b.shop_id
     GROUP BY b.shop_id, s.shop_name, b.code`,
    [dari, sampai, shopId]);
  ...
  return rows.map((r) => {
    ...
    const omzetBelum = Number(r.omzet_belum) || 0;
    const biaya      = (Number(r.biaya_nyata) || 0) + omzetBelum * tarif.tarif(r.shop_id);

    const laba   = omzet - hpp - biaya;
    const margin = omzet ? laba / omzet : 0;
    const butaHpp = hpp <= 0 && omzet > 0;

    // Urutan pemeriksaan penting: HPP nol harus menang atas "rugi" maupun
    // "tipis", karena selama HPP belum ada, laba dan margin-nya belum
    // bermakna dan tidak boleh dipakai menyimpulkan apa pun.
    const jenis = butaHpp ? 'buta'
      : laba < 0 ? 'rugi'
      : margin < ambangTipis ? 'tipis'
      : 'sehat';

    return {
      shopId: String(r.shop_id),
      toko: r.shop_name || `Toko ${r.shop_id}`,
      code: r.code, name: r.name, gambar: r.gambar,
      tertaut: !!r.tertaut,
      unit, baris: Number(r.baris) || 0,
      omzet, hpp, biaya, laba, margin,
      perUnit: unit ? laba / unit : 0,
      jenis,
    };
  });
}

/** Beban iklan per toko — total resmi, dipakai sebagai pembanding. */
export async function iklanPerToko({ dari, sampai, shopId = null }) {
  const rows = await q(
    `SELECT shop_id, COALESCE(SUM(expense),0) AS pokok
       FROM ad_spend
      WHERE day >= ($1::timestamptz AT TIME ZONE 'Asia/Jakarta')::date
        AND day <= (($2::timestamptz AT TIME ZONE 'Asia/Jakarta') - interval '1 second')::date
        AND ($3::bigint IS NULL OR shop_id = $3)
  ...
  const peta = {};
  for (const r of rows) {
    const pokok = Math.abs(Number(r.pokok) || 0);
    peta[String(r.shop_id)] = pokok * (1 + PPN_IKLAN());
  }
  return peta;
}
 ...
 * ── Biaya iklan PER SKU ──
 *
 * Shopee memberi biaya iklan per item_id (lihat lib/iklan-item.js), jadi
 * biaya di sini BUKAN pembagian rata dari total toko — tiap rupiah memang
 * milik produk itu.
 *
 * KUNCINYA order_items.item_id, BUKAN tabel products.
 *
 * Versi pertama fungsi ini memetakan item_id lewat katalog (products +
 * product_models), dan hasilnya kolom iklan kosong semua. Sebabnya kode SKU
 * di laporan ini diturunkan dari ITEM PESANAN, dengan model_sku dan item_sku
 * dicocokkan ke sku_mapping secara TERPISAH (lihat CTE `tertaut` di atas).
 * Menurunkannya dari katalog dengan COALESCE menghasilkan kode yang berbeda,
 * jadi barisnya tidak pernah bertemu. Karena order_items menyimpan item_id —
 * kunci yang sama dengan yang dipakai iklan — pemetaannya dilakukan di sini,
 * memakai persis logika yang sama dengan skuBocor.
 *
 * Akibatnya yang perlu disadari: produk yang DIIKLANKAN TAPI TIDAK LAKU
 * sama sekali pada periode ini tidak punya baris pesanan, jadi biayanya
 * tidak bisa ditempelkan ke SKU mana pun. Itu memang benar — SKU-nya juga
 * tidak muncul di tabel ini karena tabelnya dibangun dari penjualan. Biaya
 * itu masuk ke "tidak terpetakan", dan justru di situ ia paling perlu
 * dilihat: uang iklan yang tidak menghasilkan penjualan sama sekali.
 *
 * Untuk item yang variasinya tertaut ke beberapa Master SKU berbeda, biaya
 * dibagi menurut PORSI OMZET tiap kode pada periode itu — bukan rata,
 * karena omzetnya sudah diketahui dan menimbang dengan angka nyata lebih
 * jujur daripada membagi sembarang.
 */
export async function iklanPerSku({ dari, sampai, shopId = null }) {
  const rows = await q(`
    WITH dasar AS (
      SELECT o.shop_id, oi.item_id, oi.qty, oi.price, oi.item_sku, oi.model_sku
        FROM orders o
        JOIN order_items oi ON oi.order_id = o.id
       WHERE o.created_time >= $1 AND o.created_time < $2
         AND ($3::bigint IS NULL OR o.shop_id = $3)
    ...
    -- Persis seperti CTE tertaut di skuBocor: model_sku dan item_sku
    -- dicocokkan TERPISAH, tidak di-COALESCE jadi satu.
    berkode AS (
      SELECT d.shop_id, d.item_id,
             COALESCE(ms.code,
                      NULLIF(TRIM(d.model_sku),''),
                      NULLIF(TRIM(d.item_sku),''), '(tanpa SKU)') AS code,
             SUM(d.qty * d.price) AS omzet
        FROM dasar d
        LEFT JOIN sku_mapping mm ON mm.shop_id = d.shop_id AND mm.shop_sku = d.model_sku
        LEFT JOIN sku_mapping mi ON mi.shop_id = d.shop_id AND mi.shop_sku = d.item_sku
    ...
    bobot AS (
      SELECT shop_id, item_id, code,
             -- Kalau seluruh omzet item ini nol (mis. barang hadiah),
             -- porsinya jatuh ke pembagian rata supaya biayanya tidak hilang.
             COALESCE(
               NULLIF(omzet, 0) / NULLIF(SUM(omzet) OVER (PARTITION BY shop_id, item_id), 0),
               1.0 / COUNT(*) OVER (PARTITION BY shop_id, item_id)
             ) AS w
        FROM berkode
    )
    SELECT b.shop_id, b.code, SUM(a.expense * b.w) AS pokok
      FROM ad_spend_item a
      JOIN bobot b ON b.shop_id = a.shop_id AND b.item_id = a.item_id
     WHERE a.day >= ($1::timestamptz AT TIME ZONE 'Asia/Jakarta')::date
       AND a.day <= (($2::timestamptz AT TIME ZONE 'Asia/Jakarta') - interval '1 second')::date
       AND ($3::bigint IS NULL OR a.shop_id = $3)
     GROUP BY b.shop_id, b.code`, [dari, sampai, shopId]);

  const ppn = 1 + PPN_IKLAN();
  const perSku = {};
  const terpetakan = {};
  for (const r of rows) {
    const nilai = Math.abs(Number(r.pokok) || 0) * ppn;
    perSku[`${r.shop_id}|${r.code}`] = (perSku[`${r.shop_id}|${r.code}`] || 0) + nilai;
    terpetakan[String(r.shop_id)] = (terpetakan[String(r.shop_id)] || 0) + nilai;
  }
  return { perSku, terpetakan };
}

/**
 * Kenapa sebagian biaya iklan tidak menempel ke SKU. Dipakai halaman SKU
 * untuk menerangkan selisihnya, dan untuk melacak kalau kolomnya kosong.
 */
export async function iklanTakMenempel({ dari, sampai, shopId = null }) {
  return q(`
    WITH terjual AS (
      SELECT DISTINCT o.shop_id, oi.item_id
        FROM orders o
        JOIN order_items oi ON oi.order_id = o.id
       WHERE o.created_time >= $1 AND o.created_time < $2
         AND COALESCE(o.status,'') NOT IN ('CANCELLED','UNPAID')
    ...
    SELECT a.shop_id, a.item_id,
           COALESCE(SUM(a.expense), 0) AS pokok,
           MAX(p.name)     AS nama,
           MAX(p.item_sku) AS sku,
           BOOL_OR(t.item_id IS NOT NULL) AS pernah_terjual
      FROM ad_spend_item a
      LEFT JOIN terjual t  ON t.shop_id = a.shop_id AND t.item_id = a.item_id
      LEFT JOIN products p ON p.shop_id = a.shop_id AND p.item_id = a.item_id
     WHERE a.day >= ($1::timestamptz AT TIME ZONE 'Asia/Jakarta')::date
       AND a.day <= (($2::timestamptz AT TIME ZONE 'Asia/Jakarta') - interval '1 second')::date
       AND ($3::bigint IS NULL OR a.shop_id = $3)
     GROUP BY a.shop_id, a.item_id
    HAVING BOOL_OR(t.item_id IS NOT NULL) IS NOT TRUE
     ORDER BY 3 DESC
     LIMIT 20`, [dari, sampai, shopId]);
}

/** Ringkasan isi ad_spend_item — dipakai untuk membedakan "belum ditarik"
 *  dari "sudah ditarik tapi tidak menempel". */
export async function ringkasIklanItem({ dari, sampai, shopId = null }) {
  return one(`
    SELECT COUNT(*)::int AS baris,
           COUNT(DISTINCT item_id)::int AS item,
           COALESCE(SUM(expense), 0) AS pokok,
           MIN(day) AS terawal, MAX(day) AS terakhir
      FROM ad_spend_item
     WHERE day >= ($1::timestamptz AT TIME ZONE 'Asia/Jakarta')::date
       AND day <= (($2::timestamptz AT TIME ZONE 'Asia/Jakarta') - interval '1 second')::date
       AND ($3::bigint IS NULL OR shop_id = $3)`, [dari, sampai, shopId]);
}
